...
SocketBridge = {
    ...
    init: function () {
        ...
        conn.onOpen = function (e) {
            ...
            conn.controller("sequencer").subscribe("sec", function (data) {
                ...
                console.log('####$$$$#### MIDI.isUnlocked = ', MIDI.isUnlocked);
                console.log('####$$$$#### MIDI.unlocked = ', MIDI.unlocked);
                console.log('####$$$$#### mobile_midi_unlocked = ', mobile_midi_unlocked);

                if (mobile_midi_unlocked)
                {
    ...
    //    var msg = { RoomKey: roomKey, From: userFirstName, FromUserId: user, Content: msg, Attachment: null, DateSent: (new Date()).toJSON(), Type: type };
    ...
    updateTranposeUI: function (key, cmd) {
        ...
        $("#tranposeRepetitions").children().each(function (index) {
            ...
            else if (index > 0 && index < (transposeRepLength - 1) && transposeRepLength <= maxKeysToShow) { $(this).attr("class", "inactive"); }
            else if (index > 1 && index < (transposeRepLength - 1) && transposeRepLength > maxKeysToShow) { $(this).attr("class", "inactive"); }
            else if (index == (transposeRepLength - 1)) { $(this).attr("class", "active"); }

            if (transposeRepLength > maxKeysToShow) {
    ...
    drawVexflow: function (midinotes, chordnotes) {
        ...
        if (lowestNote < 40 && highestNote > 60) {
            var padTop = highestNote - 50;
            $("#tools-line-" + id).next().css("padding-top", padTop + "px");
            renderer.resize(700, 190 + Math.abs(lowestNote) + padTop);
        }

        else if (highestNote >= 60) {
            var padTop = highestNote - 50;
            $("#tools-line-" + id).next().css("padding-top", padTop + "px");
            renderer.resize(700, 190 + padTop);
        }

        else if (lowestNote < 40) {
            renderer.resize(700, 190 + Math.abs(lowestNote));
        }

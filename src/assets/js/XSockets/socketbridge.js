...
SocketBridge = {
    ...
    init: function() {
        ...
        conn.onOpen = function(e) {
            ...
            conn.controller("message").subscribe("chatmessage", function(data) {
                ...
                if (hour == 12) { ampm = "pm"; } else if (hour > 12) { hour -= 12;
                    ampm = "pm"; }
            ...
            conn.controller("message").subscribe("changeuserroomstatus", function(data) {
                ...
                if (SocketBridge.chatRoomCurrentUsers[cu_userid] != undefined) {
                    ...
                    //user_"
                    $("#currentUsers").append('<li class="' + data.Status + '" id="' + cu_userid + '" data-usertype=' + data.UserType + '><a>' + data.UserFirstName + '</a></li>');
                    SocketBridge.chatRoomCurrentUsers[cu_userid] = data.UserType;
                }
                for (var key in SocketBridge.chatRoomCurrentUsers) {
                    if (SocketBridge.chatRoomCurrentUsers[key] == "Teacher") { SocketBridge.hasTeacher = true; } else if (SocketBridge.chatRoomCurrentUsers[key] == "Student") { SocketBridge.hasAtLeastOneStudent = true; }
                }
                console.log("hasTeacher = ", SocketBridge.hasTeacher);
                console.log("hasAtLeastOneStudent = ", SocketBridge.hasAtLeastOneStudent);
                if (SocketBridge.hasTeacher && SocketBridge.hasAtLeastOneStudent) {
            ...
            setTimeout(SocketBridge.changeUserRoomStatus("in"), 5000);
            setTimeout(SocketBridge.changeUserRoomStatus("in"), 30000);
            ...
            conn.controller("sequencer").subscribe("sec", function(data) {
                ...
                if (data.EndExerciseChord1 != null)
                    SocketBridge.endExerciseChord = data.EndExerciseChord1.split(",");
                else
    ...
    addExistingUsersToRoom: function(currentUsers) {
        for (var i = 0; i < currentUsers.length; i++) {
            console.log('addExistingUsersToRoom currentUsers[' + i + '] = ', currentUsers[i]);
            var uid = currentUsers[i].UserId;
            //console.log("uid = " + uid);
            var cu_userid = "user_" + uid;
            console.log("addExistingUsersToRoom changeuserroomstatus = " + cu_userid + " SocketBridge.chatRoomCurrentUsers[cu_userid] = " + SocketBridge.chatRoomCurrentUsers[cu_userid]);
            //if (cu_userid in SocketBridge.chatRoomCurrentUsers) {
            if (SocketBridge.chatRoomCurrentUsers[cu_userid] != undefined) {
                ...
                SocketBridge.chatRoomCurrentUsers[cu_userid] = currentUsers[i].UserType;
            }
            for (var key in SocketBridge.chatRoomCurrentUsers) {
                if (SocketBridge.chatRoomCurrentUsers[key] == "Teacher") { hasTeacher = true; } else if (SocketBridge.chatRoomCurrentUsers[key] == "Student") { hasAtLeastOneStudent = true; }
            }
            console.log("hasTeacher = ", SocketBridge.hasTeacher);
            console.log("hasAtLeastOneStudent = ", SocketBridge.hasAtLeastOneStudent);
            if (SocketBridge.hasTeacher && SocketBridge.hasAtLeastOneStudent) {
    ...
    sendChatMessage: function(msg, type) {
        ...
        var msg = { RoomKey: roomKey, From: userFirstName, FromUserId: user, Content: msg, Attachment: null, DateSent: dateJson, Type: type };
    ...
    updateTranposeUI: function(key, cmd) {
        ...
        $("#tranposeRepetitions").children().each(function(index) {
            ...
            if (index == 0) { $(this).attr("class", "first"); } else if (index > 0 && index < (transposeRepLength - 1) && transposeRepLength <= 7) { $(this).attr("class", "inactive"); } else if (index > 1 && index < (transposeRepLength - 1) && transposeRepLength > 7) { $(this).attr("class", "inactive"); } else if (index == (transposeRepLength - 1)) { $(this).attr("class", "active"); }

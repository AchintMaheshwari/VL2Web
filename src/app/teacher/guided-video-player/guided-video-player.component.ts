...
@Component({
  selector: 'app-guided-video-player',
  templateUrl: './guided-video-player.component.html',
  styleUrls: ['./guided-video-player.component.scss']
})

export class GuidedVideoPlayerComponent implements OnInit, OnDestroy {
  ngOnDestroy(): void {
  ...
  constructor(private mScrollbarService: MalihuScrollbarService, private activatedRoute: ActivatedRoute, private _ngZone: NgZone,
    private crudService: CrudService, public guidedvideoService: GuidedvideoService, public dialog: MatDialog,
    public rateTeacherService: RateTeacherService, public share: ShareButtons, public commonService: CommonService) {
  }
  ...
  ngOnInit() {
    if (this.guidedvideoService.guidedLessonGuid == null || this.guidedvideoService.guidedLessonGuid == undefined || this.guidedvideoService.guidedLessonGuid == "") {
      this.guidedvideoService.guidedLessonGuid = this.activatedRoute.snapshot.params["lessonId"];
    }
  ...
  getLessonByLessonGuid() {
    if (this.guidedvideoService.guidedLessonGuid != undefined) {
  ...
  getTeacherDataByLessonGuid() {
    this.crudService.get('/guidedVideo/GetTeacherDataByLessonGuid?lessonGuid=' + this.guidedvideoService.guidedLessonGuid, null).subscribe((response: any) => {
      ...
      if (this.guidedvideoService.IsPaid == false) {
        this.guidedvideoService.getGuidedVideoCostDetailsbySource(Source, this.guidedvideoService.videoSubmissionId).subscribe((response: any) => {

...
(function () {
    ...
    midi.BPM = 120;
    midi.transpose = 0;
    midi.newTranspose = -100;
    midi.repeat = 0;
    midi.repetition = '';
    midi.repetitionIndex = 0;
    midi.repetitionArray = null;
    midi.chordMidi = null;
    midi.firstRepeat = false;
    midi.debug = false;
    midi.onStart = null;
    midi.onRedraw = null;
    midi.mute = false;
    midi.isUnlocked = false;
    midi.onCompleted = null; //we will add completed playing complete callback to this property
    midi.data = null;
    ...
    midi.start =
        midi.resume = function (onsuccess) {
            ...
            if (midi.onStart != null) { midi.onStart(); }
        };
    ...
    midi.loadFile = function (file, onsuccess, onprogress, onerror) {
        ...
        if (file.indexOf('base64,') !== -1) {
            ...
            midi.mdata = midi.loadMidiFile(onsuccess, onprogress, onerror);
        } else {
    ...
    var scheduleTracking = function (channel, note, currentTime, offset, message, velocity, time) {
        return setTimeout(function () {
            ...
            if (eventQueue.length == 0) {
                //check if we have repetition string
                if (midi.repetition.length > 0 && midi.repetitionArray == null) {
                    //split into array
                    midi.repetitionArray = midi.repetition.split(',');
                }

                if (midi.repetitionArray != null) {
                    if (midi.repetitionIndex < midi.repetitionArray.length) {
                        //transpose
                        midi.transpose += parseInt(midi.repetitionArray[midi.repetitionIndex]);
                        //increment rep index
                        midi.repetitionIndex++;
                        //reset time
                        //midi.currentTime = 0;
                        //start again
                        //midi.start();
                ...
                if (midi.repeat > 0) {
                    ...
                    if (midi.newTranspose != -100) {

                        if (midi.debug) console.log('newTranspose = ', midi.newTranspose);
                        midi.transpose = midi.newTranspose;
                        midi.newTranspose = -100;
                    }

                    if (midi.debug) console.log('repeat start');

                    if (!midi.firstRepeat) {
                        if (midi.debug) console.log('*********firstRepeat');
                        //change the data to use the ChordMidi
                        var mididata = window.atob(midi.chordMidi.split(',')[1]);
                        midi.currentData = mididata;
                        midi.replayer = new Replayer(MidiFile(midi.currentData), midi.timeWarp, null, midi.BPM);
                        //if (midi.data != undefined && midi.data != null)
                            midi.data = midi.replayer.getData();
                        midi.endTime = getLength();
                        if (midi.debug) console.log('midi.data = ', midi.data);

                        midi.firstRepeat = true;
                    }
                    midi.start();

                    if (midi.onRedraw != null) {
                        midi.onRedraw();
                    }
                } else {
                    midi.repetitionIndex = 0;

...
const CONSUMER_KEY = 'WkNWg9dF4K4B8LvMQbTyZwHZp'; //'AZ2Yl9KIWL99AHz6zojxgkdfr' //KEYS["oauth_consumer_key"];
const CONSUMER_SECRET = 'Q3UjuiiZI7KF0YQGe7rnAjD46wtvWnNUgkn7aVvoiEiRgzCTFe'; //'mHmudwbBktjfhChYeTv3V1rDiGi2dqiKYjYQwrZVUSvYz4oZK9'; // KEYS["oauth_consumer_secret"];
...
export class TwitterService {
  ...
  getOrPost(method, url, token, tokenSecret, bodyParams): Promise<any> {
    ...
    if (method == 'GET') {
      ...
      let headers = new HttpHeaders({'Authorization': authValue, 'Content-Type': 'application/x-www-form-urlencoded'});
      var body = "";
      bodyParams.forEach(function (item) {
        if (body != "") body = body + '&';
      ...
      return this.http.post(url, body, {headers: headers}).toPromise()         /* try

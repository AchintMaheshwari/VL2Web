var VoicelessonsVideoPlayer = VoicelessonsVideoPlayer || {};
VoicelessonsVideoPlayer = {
    http: null,
    sanitizer: null,
    validYouTubeOptions: [
        'default',
    ...
    init: function (element, url,tittle, options,guidedVideo=false) {
        ...
        let strHtml = this.drawPlayerTool(element,guidedVideo);        
         if (url.includes('.mp4') && !(!!window.chrome && !!window.chrome.webstore)) {
            ...
            let embedUrl = this.embed(url, options,element);
            strHtml += embedUrl
            $(element).html(strHtml);
        }
    ...
    embed: function (url, options,element=null) {
        var id;
        url = new URL(url);
        ...
        if (id) {
            return this.embed_Wistia(id, options);
        }
        else {
            return " <video width='95%' controls> <source src='" + url.href + "' type='video/mp4'></video>";
    ...
    embed_youtube: function (id, options) {
        ...
        return this.sanitize_iframe('<iframe width="500px" height="270px" src="https://www.youtube.com/embed/'
    ...
    embed_youtube_image: function (id, options) {
        ...
        options = options || {};
        options.image = voicelessonsVideoPlayer.validYouTubeOptions.indexOf(options.image) > 0 ? options.image : 'default';
    ...
    embed_vimeo_image: function (id, options) {
        ...
        options = options || {};
        options.image = voicelessonsVideoPlayer.validVimeoOptions.indexOf(options.image) >= 0 ? options.image : 'thumbnail_large';
    ...
    embed_dailymotion_image: function (id, options) {
        ...
        options = options || {};
        options.image = voicelessonsVideoPlayer.validDailyMotionOptions.indexOf(options.image) >= 0 ? options.image : 'thumbnail_480_url';
